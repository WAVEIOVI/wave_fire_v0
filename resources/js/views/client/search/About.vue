...
const onDecode = text => {
  ...
  const foundEquipment = equipments.value.find(eq => eq.qrc_id === text)
  if (foundEquipment) {
    searchQuery.value = foundEquipment.id
    fetchEquipmentAndOpenDialog()
  } else {
    console.error('Equipment not found for QR code:', text)
...
const fetchEquipmentAndOpenDialog = () => {
  if (searchQuery.value) {
    fetchEquipment()
    isViewEquipmentDialogVisible.value = true
  }
}
</script>
      ...
      <VCardText>
          ...
          <VCardText>
            ...
            <div v-if="searchQuery">
              <AppAutocomplete
                ...
                item-value="id"
                clear-icon="tabler-x"
                @update:model-value="fetchEquipmentAndOpenDialog"
              >
        ...
        <VCol cols="12">
          <VCardText>
            <VBtn
              block
              @click="fetchEquipmentAndOpenDialog"
            >
              Search Equipment 
            </VBtn>
          </VCardText>
