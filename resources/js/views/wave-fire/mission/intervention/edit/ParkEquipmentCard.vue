...
import EditEquipmentDialog from './park-dialog-statistics/EditEquipmentTasksDialog.vue'
...
const emit = defineEmits(['update:triggerChildFunction'])

// ... (keep the existing setup code, including refs, computed properties, and methods) ...
...
const updateInspectedEquipment = async equipmentData => {
  ...
  try {
    const response = await interventionListStore.updateInspectedEquipment(Number(route.params.id), equipmentId, equipmentData)
...
const deleteEquipment = equipmentId => {
  interventionListStore.detachInterventionEquipment(Number(route.params.id), equipmentId)
    .then(response => {
                  ...
                  <VCard>
                    <VCardText>
                      <VRow>
                        ...
                        <VCol
                          v-if="item.raw.pivot.equipment_protection_name && item.raw.pivot.equipment_protection_name.length > 0"
                          class="mb-0 pb-0"
                          cols="6"
                        >
                          <h4>Protection</h4>
                          <VChip
                            ...
                            class="text-sm text-medium-emphasis mb-0 me-1 text-uppercase"
                          >
                            {{ item.raw.pivot.equipment_protection_name }}
                          </VChip>
                        </VCol>
                      </VRow>
                    </VCardText>
